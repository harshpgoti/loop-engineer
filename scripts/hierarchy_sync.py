...
- **Authored product state** (`DOUBTS.md`, `HANDOFF.md`, decisions/specs/tasks) is
  never written across a workspace boundary. Generated hierarchy views
  (`PARENT_CONTEXT.md`, `SUBPRODUCTS.md`) may be regenerated by `tree_sync`.
...
def manifest_block(workspace: Path, result: dict) -> list[str]:
    ...
    if result.get("parent"):
        ...
        lines.append(
            "- **Action in this command:** resolve derived parent findings and materialize "
            "accepted constraints in this workspace's plan/spec/tasks before continuing."
        )
    if result.get("children"):
        ...
        refreshed = len(result.get("children_refreshed") or [])
        if refreshed:
            lines.append(
                f"- Generated `PARENT_CONTEXT.md` refreshed automatically for {refreshed} linked sub-product(s)."
            )
        if result.get("children_errors"):
            lines.append(
                f"- **Sync warning:** {len(result['children_errors'])} child context refresh(es) failed; "
                "see session output."
            )
    return lines
