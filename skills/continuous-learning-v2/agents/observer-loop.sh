...
ANALYSIS_COOLDOWN="${LOOP_ENGINEER_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${LOOP_ENGINEER_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${LOOP_ENGINEER_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set LOOP_ENGINEER_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${LOOP_ENGINEER_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  ...
  analysis_file="$(mktemp "${observer_tmp_dir}/loop-engineer-observer-analysis.jsonl.XXXXXX")"
  ...
  # agent harness resolves relative paths against the user's home directory on
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/loop-engineer-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${LOOP_ENGINEER_OBSERVER_TIMEOUT_SECONDS:-120}"
  ...
  if [ -n "${LOOP_ENGINEER_OBSERVER_MAX_TURNS:-}" ]; then
    max_turns="${LOOP_ENGINEER_OBSERVER_MAX_TURNS}"
  else
  ...
  # but an explicit LOOP_ENGINEER_OBSERVER_MAX_TURNS override may be non-numeric, empty, or too
  ...
  # Model is configurable via LOOP_ENGINEER_OBSERVER_MODEL (defaults to haiku for cost efficiency);
  # e.g. LOOP_ENGINEER_OBSERVER_MODEL=opus for higher-quality instinct extraction. Heavier models are
  # slower — consider raising LOOP_ENGINEER_OBSERVER_TIMEOUT_SECONDS (default 120s) so the watchdog
  # doesn't kill the analysis mid-run.
  LOOP_ENGINEER_SKIP_OBSERVE=1 LOOP_ENGINEER_HOOK_PROFILE=minimal claude --model "${LOOP_ENGINEER_OBSERVER_MODEL:-haiku}" --max-turns "$max_turns" --print \
