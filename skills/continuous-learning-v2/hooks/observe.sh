...
# agent harness passes hook data via stdin as JSON.
#
...
# Can also be registered manually in <agent-config-root>/settings.json.

set -e

# Hook phase from CLI argument: "pre" (PreToolUse) or "post" (PostToolUse).
# Manual settings.json installs can call this script without the plugin
# wrapper's positional phase argument, but agent harness still exposes the hook
...
# Read JSON from stdin (agent harness hook format)
INPUT_JSON=$(cat)
...
#   - Loop Engineer observing its own Haiku observer sessions (self-loop)
#   - Loop Engineer observing other tools' automated sessions
#   - automated sessions creating project-scoped instinct-store metadata
...
# (LOOP_ENGINEER_HOOK_PROFILE=minimal, LOOP_ENGINEER_SKIP_OBSERVE=1, agent_id, path exclusions).
...
[ "${LOOP_ENGINEER_HOOK_PROFILE:-standard}" = "minimal" ] && exit 0

# Layer 3: cooperative skip env var for automated sessions.
[ "${LOOP_ENGINEER_SKIP_OBSERVE:-0}" = "1" ] && exit 0

# Layer 4: subagent sessions are automated by definition.
_LOOP_ENGINEER_AGENT_ID=$(echo "$INPUT_JSON" | "$PYTHON_CMD" -c "import json,sys; print(json.load(sys.stdin).get('agent_id',''))" 2>/dev/null || true)
[ -n "$_LOOP_ENGINEER_AGENT_ID" ] && exit 0

# Layer 5: known observer-session path exclusions.
_LOOP_ENGINEER_SKIP_PATHS="${LOOP_ENGINEER_OBSERVE_SKIP_PATHS:-observer-sessions,.claude-mem}"
if [ -n "$STDIN_CWD" ]; then
  IFS=',' read -ra _LOOP_ENGINEER_SKIP_ARRAY <<< "$_LOOP_ENGINEER_SKIP_PATHS"
  for _pattern in "${_LOOP_ENGINEER_SKIP_ARRAY[@]}"; do
...
# Pass HOOK_PHASE via env var since agent harness does not include hook type in stdin JSON
...
try:
    ...
    # agent harness does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - agent harness hook format
...
_NOTE_OBSERVER_NOSURVIVE() {
  ...
  local warn_after="${LOOP_ENGINEER_OBSERVER_NOSURVIVE_WARN_AFTER:-3}"
  local streak
  ...
  if printf '%s\n' "$streak" > "$streak_file" 2>/dev/null &&
    ...
    printf -v message '%s\n%s\n%s\n%s' \
      ...
      "$platform_hint" \
      "[observe] Set LOOP_ENGINEER_OBSERVER_NOSURVIVE_WARN_AFTER to change this threshold (currently ${warn_after})."
...
# which caused runaway parallel agent analysis processes.
SIGNAL_EVERY_N="${LOOP_ENGINEER_OBSERVER_SIGNAL_EVERY_N:-20}"
