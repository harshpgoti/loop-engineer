#!/usr/bin/env bash
# Loop Engineer: one-shot migration of legacy instinct-store data into the
# continuous-learning-v2 data directory. Maintained by the Loop Engineer team.
set -euo pipefail

OLD="${HOME}/.agents/homunculus"
...
if command -v pgrep >/dev/null 2>&1; then
  ...
  if pgrep -f "${escaped_home}.*observer-loop\\.sh" >/dev/null 2>&1; then
    ...
    echo "Exit all agent harness sessions, then re-run." >&2
    exit 1
  fi
else
...
settings="${HOME}/.agents/settings.json"
if [ -f "$settings" ] && grep -q '"CLV2_CONFIG"' "$settings" 2>/dev/null; then
  if grep -q '\.agents/homunculus' "$settings" 2>/dev/null; then
    cat >&2 <<WARN

Advisory: <agent-config-root>/settings.json still sets CLV2_CONFIG under the old path.
