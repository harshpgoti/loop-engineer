...
def _resolve_homunculus_dir() -> Path:
    ...
    if override:
        ...
        print(f"[loop-engineer] CLV2_HOMUNCULUS_DIR={override!r} is not absolute; ignoring", file=sys.stderr)

    xdg = os.environ.get("XDG_DATA_HOME")
    if xdg:
        if Path(xdg).is_absolute():
            return Path(xdg) / "loop-engineer-homunculus"
        print(f"[loop-engineer] XDG_DATA_HOME={xdg!r} is not absolute; ignoring", file=sys.stderr)

    return Path.home() / ".local" / "share" / "loop-engineer-homunculus"
...
def _fetch_import_url(source: str, *, max_bytes: int = 2 * 1024 * 1024) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Loop Engineer-instinct-import/2"})
...
def _warn_legacy_data() -> None:
    """Warn if legacy <agent-config-root>/instinct-store/ contains data while the active
...
def cmd_projects(args) -> int:
    ...
    if not registry:
        ...
        print("Projects are auto-detected when you use agent harness in a git repo.")
        return 0
...
def _evolved_description(trigger: str, instincts: list, kind: str) -> str:
    ...
    agent harness (and every spec-compliant Agent Skills client) injects only
