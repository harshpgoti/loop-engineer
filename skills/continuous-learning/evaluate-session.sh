...
#
# Hook config (in <agent-config-root>/settings.json):
# {
#   "hooks": {
#     "Stop": [{
#       "matcher": "*",
...
# Extracted skills saved to: <agent-config-root>/skills/learned/
...
if [ -f "$CONFIG_FILE" ]; then
  if ! command -v jq &>/dev/null; then
    ...
    LEARNED_SKILLS_PATH=$(jq -r '.learned_skills_path // "<agent-config-root>/skills/learned/"' "$CONFIG_FILE" | sed "s|~|$HOME|")
  fi
fi
...
# Get transcript path from stdin JSON (agent harness hook input)
