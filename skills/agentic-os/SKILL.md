---
name: agentic-os
description: Build persistent multi-agent operating systems on agent harness. Covers kernel architecture, specialist agents, slash commands, file-based memory, scheduled automation, and state management without external databases. Use when building a persistent multi-agent system on agent harness with its own memory, commands, and scheduling.
metadata:
...
Treat agent harness as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: a kernel config that routes tasks to specialist agents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## When to Activate

- Building a multi-agent workflow inside agent harness
- Setting up persistent agent harness automation that survives session restarts
...
├── .agents/commands/  # Slash commands: user-facing CLI
...
| Commands (`.agents/commands/`) | User-facing slash commands (`/daily-sync`, `/outreach`) | Git-tracked |
...
`AGENTS.md` is the kernel. It acts as the COO / orchestrator. The host agent reads it at session start and uses it to route work.
...
Each agent is a standalone markdown file in `agents/`. The host agent loads the relevant agent file when routing a task.
...
- Test runner access
- MCP servers as configured in `.agents/mcp.json`
...
For parallel execution, use agent harness's background task capability or shell scripts that invoke agent harness with specific agent contexts.

## Commands and Daily Workflows

Slash commands are markdown files in `.agents/commands/`. They define reusable workflows.
...
Place command files in `.agents/commands/<command-name>.md`. agent harness auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not agent harness's built-in cron (which dies when the session ends).
...
// data/projects/website-v2.json
{
  ...
  "files": {
    "spec": "<product-root>/docs/website-v2-spec.md",
...
Starting fresh every time agent harness opens.
```
...
Your provider API key is sk-xxxxxxxx
```
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not agent harness's session cron
