...
```python
# Spawn reviewers in parallel (agent harness subagents)
...
### Pattern A: agent harness Subagents (Recommended)

Subagents provide true context isolation. Each reviewer is a separate process with no shared state.

```bash
# In a agent harness session, use the Agent tool to spawn reviewers
