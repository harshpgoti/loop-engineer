...
- the in-context host-agent voice
- a Skeptic subagent
...
Do **not** write ad-hoc notes to `<agent-config-root>/notes` or other shadow paths from this skill.
...
```text
Should we ship Loop Engineer 2.0 as alpha now, or hold until the control-plane UI is more complete?
```
