...
Loop Engineer can plan and build **any** product, including one that is, or includes, an AI agent - a chatbot, autonomous workflow, tool-using assistant, multi-agent system, or dynamic/branching automation. This capability is auto-detected during `/plan-loop` and `/develop-product`, and can be run directly with `/agent-builder`.
...
| `/develop-product` | Reads `plan/AUTO_AGENT_SKILLS.md` alongside `plan/AUTO_SKILLS.md`; runs `agent-builder` for the agent-loop development domain |
