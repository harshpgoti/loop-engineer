...
```bash
loop manage-model models anthropic              # live list from Anthropic API
loop manage-model models openai --search gpt    # filter results
loop manage-model anthropic:claude-opus-4-20250514   # any model id the provider accepts
loop manage-model openrouter:anthropic/claude-opus-4
```

`default_model` in the registry is only a **fallback** when you omit the model:

```bash
loop manage-model anthropic    # uses registry fallback if you skip the model suffix
```
...
loop manage-model custom add work https://gpu-server.internal/v1 --key-env CORP_API_KEY
...
```bash
loop manage-model custom:local:qwen-2.5
loop manage-model custom:work:llama-3.3-70b
```
...
loop manage-model fallback add anthropic:claude-sonnet-4-20250514
loop manage-model fallback list
loop manage-model fallback clear
```
...
loop manage-model context show
```
...
| Perplexity, MiniMax | Marked `note:` **unconfirmed** in the registry - their current docs didn't clearly show a plain `/v1/chat/completions`-style path during the last verification pass. Confirm the exact path against the provider's docs before relying on `loop manage-model models` for these two. |
...
- Named custom providers (`loop manage-model custom add`) store `base_url` and `env_key` name in `model.yml` (safe to share) - the actual key value still only lives in `secrets.env`.

## Troubleshooting

| Symptom | Fix |
|---------|-----|
| `missing OPENROUTER_API_KEY` | `loop manage-model set-key OPENROUTER_API_KEY` |
| `loop manage-model models` fails | Check key, base URL, provider docs for model id |
| Local probe fails | Start Ollama/LM Studio/vLLM/etc.; confirm base URL |
| Named custom provider `Unknown custom provider` | `loop manage-model custom add <name> <base_url>` first, then select `custom:<name>:<model>` |
